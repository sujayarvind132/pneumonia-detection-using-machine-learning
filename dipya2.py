...
def upload_image():
    ...
    if not is_bw_image(file_path):
        result_label.configure(text="Please give valid input", font=("Arial", 18, "bold"), fg="red")
        return
...
def predict_image():
    ...
    if prediction[0] < prediction[1]:
        result_label.configure(text=f"Prediction for {selected_img_name}: {percentage_pneumonia}% Pneumonia is Present",
